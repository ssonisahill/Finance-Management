    ...
    <PageContainer>
      ...
      <div className="bg-card border border-border rounded-3xl p-6 shadow-sm">
        ...
        <div className="space-y-3">
          <h3 className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Active Subscriptions ({subscriptions.length})</h3>
          {subscriptions.length === 0 ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {subscriptions.map(sub => {
                ...
                return (
                  <div key={sub.id} className="flex items-center justify-between p-3.5 border border-border/60 rounded-2xl bg-muted/10 hover:border-border transition-colors">
                    <div className="min-w-0">
                      <h4 className="text-xs font-bold text-foreground truncate">{sub.name}</h4>
                      <p className="text-[10px] text-muted-foreground mt-0.5">
                        Due on Day {sub.due_day} • {acc?.name || 'Account'}
                        {cat && ` • ${cat.name}`}
                      </p>
                    </div>
